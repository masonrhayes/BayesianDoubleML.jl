...
@testset "VMP manual backend without RxInfer" begin
    ...
    fit!(model, method; n_iterations = 20, n_draws = 100, rng = Xoshiro(1))
    @test isfitted(model)
    ...
    # Default RxInfer backend should raise informative error when extension not loaded
    model2 = BDMLModel(df, :y, :d; model_type = :basic)
    method_rx = VMP()  # defaults to RxInferVMP
